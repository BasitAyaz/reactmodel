...
function TableGrid(props) {
  ...
  function searchVal() {
    let arr = dataSourse.filter((x) => x[searchKey] == searchText);
    ...
    console.log(searchText, searchKey, arr);
  }
  ...
  return (
    <div>
      <div className="d-flex justify-content-between">
        <div>
          <h3>{props.title ? props.title : "Table"}</h3>
        </div>
        <div>
      ...
      <table className="table table-responsive">
        <thead>
          <tr>
            {gridCols.map((e, i) => {
              return <th key={i}>{e.displayName}</th>;
            })}
          </tr>
        </thead>
        <tbody>
          {searchData && searchData.length > 0
            ? searchData.map((e, i) => {
                return (
                  <tr key={i}>
                    {gridCols && Array.isArray(gridCols)
                      ? gridCols.map((item, ind) => {
                          return <td key={ind}>{e[item.key]}</td>;
                        })
                      : null}
                  </tr>
                );
              })
            : dataSourse && Array.isArray(dataSourse) && !searchData.length > 0
            ? dataSourse.map((e, i) => {
                return (
                  <tr key={i}>
                    {gridCols && Array.isArray(gridCols)
